...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate an adapter-trained checkpoint on test set")
...
def main():
    ...
    model = AdapterClassifier(num_classes=num_classes, freeze_backbone=True, model_name=cfg["model_name"],
                              use_hub=cfg.get("use_hub", False), hub_name=cfg.get("hub_name", None),
                              use_registers=cfg.get("use_registers", False), img_size=cfg["img_size"],
                              adapter_hidden_ratio=cfg.get("adapter_hidden_ratio", 0.5),
                              adapter_dropout=cfg.get("adapter_dropout", 0.1),
                              adapter_use_layernorm=cfg.get("adapter_use_layernorm", True)).to(device())
    ...
    if total == 0:
        ...
        acc = correct / total
        rprint(f"[bold]Test accuracy (adapter):[/bold] {acc:.3f}  ({correct}/{total})")
